...
export function initializeSocket(server) {
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('private:create', ({ code, timeControl }) => {
      ...
      if (privateMatchRooms.has(code)) {
        return socket.emit('error', { message: 'Room already exists' });
      }

      // Create room
      privateMatchRooms.set(code, {
        creator: socket.user.wallet,
      ...
      console.log(`Private room created: ${code} by ${socket.user.wallet}`);
    });

    // Join private match room
    socket.on('private:join', ({ code, timeControl }) => {
      const room = privateMatchRooms.get(code);
      
      if (!room) {
        return socket.emit('error', { message: 'Room not found or expired' });
      }

      if (room.creator === socket.user.wallet) {
      ...
      const match = new MatchState(
        matchId,
        { wallet: room.creator, socketId: room.creatorSocketId },
        { wallet: socket.user.wallet, socketId: socket.id },
        room.timeControl,
      ...
      activeMatches.set(matchId, match);
      playerMatches.set(room.creator, matchId);
      playerMatches.set(socket.user.wallet, matchId);
      ...
      if (creatorSocket) {
        ...
        creatorSocket.leave(`private:${code}`);
      }
      ...
      console.log(`Private match started: ${matchId} (${room.creator} vs ${socket.user.wallet})`);
    });

    // Cancel private match room
    socket.on('private:cancel', ({ code }) => {
      const room = privateMatchRooms.get(code);
      
      if (room && room.creator === socket.user.wallet) {
        privateMatchRooms.delete(code);
    ...
    socket.on('private:check', ({ code }) => {
      ...
      if (room) {
        socket.emit('private:status', {
          code,
          exists: true,
          isCreator: room.creator === socket.user.wallet,
