...
// Get user from database - supports wallet, email, and userId lookups
async function getUser(identifier) {
  ...
  // Search by multiple fields to support all auth methods
...
// Update user stats after match - supports all auth methods
async function updateUserStats(identifier, isWin, isVipArena) {
  const database = await getDb();
  
  // Check if identifier looks like an email
  const isEmail = identifier && identifier.includes('@');
  
  // Find user by multiple fields
  const query = {
    $or: [
      { wallet: identifier },
      { email: isEmail ? identifier.toLowerCase() : identifier },
  ...
  // Update using the same query to ensure we update the correct user
  await database.collection('users').updateOne(query, updates);
  return rewards;
}
...
export function initializeSocket(server) {
  ...
  io.use(async (socket, next) => {
    ...
    // Support all auth methods: wallet field can contain wallet address, email, or be missing
    // Also check for userId and email fields in the token
    const identifier = payload?.wallet || payload?.email || payload?.userId;
    ...
    // Use a unified identifier for socket operations
    // This will be either the wallet address or email (used as identifier for email users)
    socket.user = {
      wallet: identifier, // Unified identifier (wallet address or email)
  ...
  io.on('connection', (socket) => {
    console.log(`Player connected: ${socket.user.wallet} (${socket.user.displayName || 'Anonymous'})`);
    ...
    socket.on('match:quickchat', ({ matchId, presetId, type }, ackCallback) => {
      console.log(`[QuickChat] ========== NEW MESSAGE ==========`);
      console.log(`[QuickChat] Sender: ${socket.user.wallet}`);
      console.log(`[QuickChat] Sender socket.id: ${socket.id}`);
      console.log(`[QuickChat] matchId: ${matchId}`);
      console.log(`[QuickChat] presetId: ${presetId}, type: ${type}`);
      
      const match = activeMatches.get(matchId);
      if (!match) {
        console.log(`[QuickChat] ❌ Match ${matchId} not found in activeMatches`);
        if (ackCallback) ackCallback({ ok: false, error: 'Match not found' });
        return socket.emit('error', { message: 'Match not found' });
      }
      if (match.status !== 'active') {
        console.log(`[QuickChat] ❌ Match ${matchId} status is ${match.status}, not active`);
        if (ackCallback) ackCallback({ ok: false, error: 'Match not active' });
        return;
      }
      ...
      if (!playerColor) {
        ...
        if (ackCallback) ackCallback({ ok: false, error: 'Not in match' });
        return;
      }
      ...
      if (chatType === 'message' && !VALID_PRESETS.includes(presetId)) {
        if (ackCallback) ackCallback({ ok: false, error: 'Invalid preset' });
        return socket.emit('error', { message: 'Invalid quick chat preset' });
      }
      if (chatType === 'emote' && !VALID_EMOTES.includes(presetId)) {
        if (ackCallback) ackCallback({ ok: false, error: 'Invalid emote' });
      ...
      if (now - lastChat < QUICKCHAT_COOLDOWN) {
        if (ackCallback) ackCallback({ ok: false, error: 'Cooldown' });
      ...
      console.log(`[QuickChat] Room ${matchId} members:`, roomMembers);
      console.log(`[QuickChat] Room members count: ${roomMembers.length}`);
      console.log(`[QuickChat] White socketId (stored): ${match.players.white.socketId}`);
      console.log(`[QuickChat] Black socketId (stored): ${match.players.black.socketId}`);
      
      // Check if both stored socketIds are in the room
      const whiteInRoom = roomMembers.includes(match.players.white.socketId);
      const blackInRoom = roomMembers.includes(match.players.black.socketId);
      console.log(`[QuickChat] White in room: ${whiteInRoom}, Black in room: ${blackInRoom}`);
      ...
      // Method 1: Broadcast to room
      io.to(matchId).emit('match:quickchat', chatPayload);
      
      // Method 2: Also emit directly to stored socket IDs
      const whiteSocket = io.sockets.sockets.get(match.players.white.socketId);
      const blackSocket = io.sockets.sockets.get(match.players.black.socketId);
      
      console.log(`[QuickChat] White socket exists: ${!!whiteSocket}, connected: ${whiteSocket?.connected}`);
      console.log(`[QuickChat] Black socket exists: ${!!blackSocket}, connected: ${blackSocket?.connected}`);
      
      if (whiteSocket?.connected) {
        whiteSocket.emit('match:quickchat', chatPayload);
        console.log(`[QuickChat] ✅ Sent directly to white socket ${match.players.white.socketId}`);
      }
      if (blackSocket?.connected) {
        blackSocket.emit('match:quickchat', chatPayload);
        console.log(`[QuickChat] ✅ Sent directly to black socket ${match.players.black.socketId}`);
      }
      ...
      if (ackCallback) {
        ackCallback({ 
          ...
          whiteConnected: !!whiteSocket?.connected,
          blackConnected: !!blackSocket?.connected
        });
      }
      
      console.log(`[QuickChat] ========== END ==========`);
    });
